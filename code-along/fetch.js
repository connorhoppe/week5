// listen for the document to be finished loading, everything else only happens after that
// Question for office hours: would you ever not want to wait for everything to load? What if on slow connection?
document.addEventListener('DOMContentLoaded', async function(event) {

  // Get a reference to the "submit" button
  let submitButton = document.querySelector(`button`) // Can do this bc it's the one and only button
  ...
  submitButton.addEventListener(`click`, async function(event){
    ...
    let response = await fetch(url) // Note that this will wait as long as it takes to get a response - could be 5 minutes if that's how long it takes to get a response
    ...
    sentence = `The total value of ${amount} Bitcoin is ${currentValue}.`
